				...
				<div class="col-sm-8">
					<form method="post" class="uedit uadd form-ocms" enctype="multipart/form-data">
						...
						<div class="form-field">
	                        ...
	                        <input name="tytul_pl" id="tytul_pl" required value="<?= $this->data['galeria']['tytul_pl']; ?>" type="text">
	                    </div>

	                    <div class="form-field">
	                        <label for="tytul_en">tytuł EN</label>
	                        <input name="tytul_en" id="tytul_en" required value="<?= $this->data['galeria']['tytul_en']; ?>" type="text">
	                    </div>

	                    <div class="form-field form-field-checkbox">
	                        <label for="stat">
	                        	<?php $checked = ( $this->data['galeria']['stat'] == 1 ) ? 'checked' : ''; ?>
	                        	<input type="checkbox" name='stat' id='stat' value='1' <?= $checked; ?>> Zaznacz aby aktywowac wpis
	                        </label>
	                    </div>
	                    <div class="form-field">
	                    	<div class="image-input">
	                    		<span>Zmień obrazek w formacie jpg lub png o szerokosci min. 900px</span>
	                    		<input type="file" name='image_upload' id='image_upload' accept=".png, .jpg, .jpeg">
	                    	</div>
	                    	<div id="image-error"></div>
	                    	<div id="image-prev">
	                    	<div id="image-prev">
	                    		<?php if( @file_get_contents( 'userfiles/images/galeria/' . $this->data['galeria']['image'] ) ) : ?>
									<img src="<?= BASE; ?>userfiles/images/galeria/<?= $this->data['galeria']['image']; ?>?<?= rand(0,25000); ?>" alt="">
								<?php endif; ?>
	                    	</div>
	                    </div>

						<br><br>
	                    <div><a href="<?= BASE; ?>admin_galeria/widok" class="page-action-link"><i class="icon-wroc"></i> Wróć do listy galerii</a> <button name="edit" class="or-btn or-link" value="1">Zapisz</button></div>
	                </form>
                </div>
