...
<div class="content admin-content">
	<div class="ctn1360">
		<h1>Edytuj artykuł: <strong><?= htmlentities( $this->data['artykul']['tytul_pl'] ); ?></strong></h1>
		<div class="row">
			<div class="wctn row">
				<div class="col-sm-8">
					<form method="post" class="uedit uadd form-ocms" enctype="multipart/form-data">
	                	<div class="form-field">
	                        ...
	                        <input required="" name="tytul_pl" id="tytul_pl" value="<?= htmlentities( $this->data['artykul']['tytul_pl'] ); ?>" type="text">
	                    </div>
	                    
						<div class="form-field">
	                        <label for="opis_pl">zajawka</label>
	                        <textarea class='wyswig' name="zajawka_pl" id="zajawka_pl"><?= htmlentities( $this->data['artykul']['zajawka_pl'] ); ?></textarea>
	                    </div>

	                    <div class="form-field">
	                        <label for="opis_pl">opis</label>
	                        <textarea class='wyswig' name="opis_pl" id="opis_pl"><?= htmlentities( $this->data['artykul']['tresc_pl'] ); ?></textarea>
	                    </div>
	                    ...
	                    <div class="form-field">
	                        <label for="issue">link issue</label>
	                        <input name="issue" id="issue" value="<?= $this->data['artykul']['issue']; ?>" type="text">
	                    </div>

	                    <div class="form-field form-field-checkbox">
	                        <label for="dostep">
	                        	<?php $checked = ( $this->data['artykul']['ograniczony_dostep'] == 1 ) ? 'checked' : ''; ?>
	                        	<input type="checkbox" name='dostep' id='dostep' value='1' <?= $checked; ?>> Ograniczony dostęp do artykułu
	                        </label>
	                    </div>
	                    <div class="form-field">
	                    	...
	                    	<div id="image-prev">
	                    		<?php if( @file_get_contents( 'userfiles/images/artykuly/' . $this->data['artykul']['image'] ) ) : ?>
									<img src="<?= BASE; ?>userfiles/images/artykuly/<?= $this->data['artykul']['image']; ?>?<?= rand(0,25000); ?>" alt="">
								<?php endif; ?>
	                    	</div>
	                    </div>

						<br><br>
	                    <div><a href="<?= BASE; ?>admin_artykuly/widok" class="page-action-link"><i class="icon-wroc"></i> Wróć do listy artykułów</a> <button name="edit" class="or-btn or-link" value="1">Zapisz</button></div>
	                </form>
                </div>
