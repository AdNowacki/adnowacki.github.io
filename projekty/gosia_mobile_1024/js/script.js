    ...
    slider = {
        ...
        setCount: function() {
            this.count = $(this.sliderName).find('.s_images').find(this.slide).length;
        },
        setWindow: function() {
        ...
        startSlider: function() {
            ...
            $(this.sliderName).find('.s_images').find(this.slide).fadeOut(400);
    ...
    kafelki = {
        ...
        addClassHorizontal: function() {
            setTimeout(function() {
                fiL = $('#filters.category .filter').length;
                for (m = 0; m < fiL; m++) {
                    iW = $('#filters.category .filter').eq(m).find('img').width();
                    iH = $('#filters.category .filter').eq(m).find('img').height();
                    if (iW >= iH)
                        $('#filters.category .filter').eq(m).find('img').addClass('horizontal');
                }
            }, 200);
        }
    };
    ...
    function curentImgHeight() {
        setTimeout(function() {
            ...
            $('.current_img').eq(0).height(Math.ceil(ciw));
        }, 150);
    };
    ...
    $(document).ready(function() {
        ...
        $('#submenu_m:not(.prod) > a:first-child').click(function(event) {
            event.preventDefault();
            if ($(this).attr('data-show') == 1) {
                $('#roll_m').slideUp(100);
                $(this).attr('data-show', '0');
                $('#submenu_m > a:first-child img').remove();
                $('#submenu_m > a:first-child').append("<img src='images/submenu2.png' alt='' />");
            } else {
                $('#roll_m').slideDown(100);
                $(this).attr('data-show', '1');
                $('#submenu_m > a:first-child img').remove();
                $('#submenu_m > a:first-child').append("<img src='images/submenu1.png' alt='' />");
            }
        });

        $('#submenu_m.prod a:first-child').click(function(event) {
            event.preventDefault();
            if ($(this).attr('data-show') == 1) {
                $('#roll_m').slideUp(100);
                $(this).attr('data-show', '0');
                $('#submenu_m.prod > a:first-child img').remove();
                $('#submenu_m.prod > a:first-child').append("<img src='images/submenu4.png' alt='' />");
            } else {
                $('#roll_m').slideDown(100);
                $(this).attr('data-show', '1');
                $('#submenu_m.prod > a:first-child img').remove();
                $('#submenu_m.prod > a:first-child').append("<img src='images/submenu3.png' alt='' />");
            }
        });
        ...
        $('#m_prev').click(function(event) {
            ...
            mGalleryInit--;
            if (mGalleryInit < 1) {
                mGalleryInit = mGalleryAll;
            }
            imgCurrent = mGalleryInit - 1;
            galeriaMobile = new galeria($('.gallery img').eq(imgCurrent), '.current_img', 'big_', 'images/');
            galeriaMobile.change();
            delete(galeriaMobile);
        });

        $('#m_next').click(function(event) {
            event.preventDefault();
            mGalleryInit++;
            if (mGalleryInit > mGalleryAll) {
                mGalleryInit = 1;
            }
            imgCurrent = mGalleryInit - 1;
            galeriaMobile = new galeria($('.gallery img').eq(imgCurrent), '.current_img', 'big_', 'images/');
        ...
        // rozpoznawanie rozmiarów zdjęć produktów i nadanie klasy horizontal dla zdjęć o szerokości > wysokości
        kafelki.addClassHorizontal();

        // submenu produkty - ustawienie wysokości
        wH = slider.windowHeight - 44 - 37;
        sH = (wH / $("[data-count]").attr('data-count'));
        sHPadding = (sH - 15) / 2;
        sH -= sHPadding * 2;
        $('div#roll_m a').height(sH).css({
            'padding-top': sHPadding + 'px',
            'padding-bottom': sHPadding + "px"
        });
