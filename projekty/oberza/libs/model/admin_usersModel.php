...
class admin_usersModel extends Model {
	...
	public $options = [ 'Table' => 'users', 'Redirect' => 'admin_users', 'SearchCol' => '',];
	...
	function widok() {
		...
		/*
		$sth = $this->pdo->prepare( "SELECT * FROM newsroom_klienci WHERE id = {$id} LIMIT 1" );
		if( !$sth->execute() )
			throw new modelException( "Wystąpił błąd z pobraniem danych", 1 );

		$this->data['biuro'] = $sth->fetch( PDO::FETCH_ASSOC );
		if( !$this->data['biuro'] )
			throw new modelException( "Biuro prasowe nie istnieje", 1);
		*/
			
		$sth = $this->pdo->prepare( "SELECT * FROM {$this->options['Table']} WHERE newsroom_klient_id IS NOT NULL {$search} ORDER BY nazwisko, imie LIMIT {$limit} OFFSET {$offset}" );
		$sth->execute();
		$this->data['users'] = $sth->fetchAll( PDO::FETCH_ASSOC );

		foreach( $this->data['users'] as &$item ) {
			$sth = $this->pdo->prepare( "SELECT * FROM firmy WHERE id = {$item['newsroom_klient_id']} LIMIT 1" );
			$sth->execute();
		...
		$sth = $this->pdo->prepare( "SELECT COUNT(*) as TOTAL FROM {$this->options['Table']} newsroom_klient_id IS NOT NULL {$search}" );
		$sth->execute();
	...
	function edytuj() {
		...
		$sth = $this->pdo->prepare( "SELECT * FROM firmy WHERE stat = '1' ORDER BY nazwa_pl" );
		$sth->execute();
		...
		if( $_POST['edit'] == 1 ) {
			$this->data['biuro'] = (int)$_POST['biuro'];
			$this->data['imie'] = trim( $_POST['imie'] );
			$this->data['nazwisko'] = trim( $_POST['nazwisko'] );
			$this->data['stat'] = $_POST['stat'] = ( $_POST['stat'] ) ? 1 : 0;

			$sth = $this->pdo->prepare( "UPDATE {$this->options['Table']} SET newsroom_klient_id = {$this->data['biuro']}, imie = :imie, nazwisko = :nazwisko WHERE id = {$id}" );
	...
	function dodaj() {
		...
		*/
		...
		if( $_POST['add'] == 1 ) {
			...
			$sth = $this->pdo->prepare( "INSERT INTO {$this->options['Table']} (newsroom_klient_id, email, haslo, imie, nazwisko, od_kiedy, uprawnienia, admin, token, stat ) VALUES( {$this->data['biuro']}, :email, '{$haslo}', :imie, :nazwisko, NOW(), :uprawnienia, '{$this->data['admin_']}', '{$token}', '{$this->data['stat']}' )" );
			$sth->execute( 
				[
			...
			if( $this->data['stat'] == 1 ) {
				...
				$_SESSION[I_SUCCESS] = "Poprawnie dodano nowego użytkownika, jednak nie został do niego wysłany email z danymi dostępowymi";
			}

			header( "Location: " . BASE . $this->options['Redirect'] . "/widok/" . $this->data['biuro'] );
			exit();
