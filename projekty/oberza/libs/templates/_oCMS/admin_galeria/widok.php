...
<div class="content admin-content">
	<div class="ctn1360">
		<h1>Zdjęcia galerii</h1>
		<div class="row">
			<div class="wctn row">
				<a href="<?= BASE; ?>admin_galeria/dodaj" class="or-btn or-link" value="1">Dodaj zdjęcie</a>
				<br><br>
				<?php 
				...
				<?php if( $this->data['galeria'] ) : ?>
					...
					<table class="t-results">
						<tr>
							...
							<th class='w20'>Tytuł PL</th>
							<th class='w20'>Tytuł EN</th>
							<th class='w10 center-a'>Podgląd obrazka</th>
					...
					<?php foreach( $this->data['galeria'] as $aData ) : ?>
						...
						<tr <?= $off; ?>>
							...
							<td class='w20'><?= $aData['tytul_pl']; ?></td>
							<td class='w20'><?= $aData['tytul_en']; ?></td>
							<td class='w10 center-a'>
								<?php if( @file_get_contents( 'userfiles/images/galeria/' . $aData['image'] ) ) : ?>
									<img src="<?= BASE; ?>userfiles/images/galeria/<?= $aData['image']; ?>?<?= rand(0,25000); ?>" alt="" width='100'>
								<?php endif; ?>
							</td>
							<td class='w10 center-a'>
								...
								<?php if( !Auth::accessDenied( $_SESSION[AUTH_SESSION_NAME]['permissions'], ['newsroom', 'user'] ) ) : ?>
									<a href="<?= BASE . Routing::$routing['controller']; ?>/usun/<?= $aData['id']; ?>" data-message='Czy na pewno chcesz usunąć zdjęcie? ' data-tab='admin_galeria' data-i='<?= $aData['id']; ?>' class='show-link show-link-off'><i class="icon-trash"></i> usuń</a>
								<?php endif; ?>
							</td>
							<td class='w10 center-a'>
								<?php if( $lp == 1 ) : ?>
